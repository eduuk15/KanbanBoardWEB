...
@router.put("/{user_id}")
async def edit_user(user_id: int, user_data: dict, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if user_data['password']:
            user.set_password(user_data['password'])
        user.confirmation_question = user_data['confirmation_question']
        user.confirmation_answer = user_data['confirmation_answer']

...
def user_in_group(group_id: int, user_id: int, db: Session):
    return db.query(user_group).filter(user_group.c.group_id == group_id, user_group.c.user_id == user_id).first() is not None
...
async def get_group(group_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not user_in_group(group_id, current_user.id, db):
        raise HTTPException(status_code=403, detail="Você não tem permissão para visualizar este grupo.")
...
async def accept_invite(
    ...
    if current_user.id != group.created_by:
        raise HTTPException(status_code=403, detail="Você não tem permissão para aceitar esse convite")
...
async def decline_invite(
    ...
    group = db.query(Group).filter(Group.id == invite.group_id).first()
    if current_user.id != group.created_by:
        raise HTTPException(status_code=403, detail="Você não tem permissão para recusar esse convite")
...
async def get_group_invites(group_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    group = db.query(Group).filter(Group.id == group_id).first()
    if current_user.id != group.created_by:
        raise HTTPException(status_code=403, detail="Você não tem permissão para visualizar os convites deste grupo")
